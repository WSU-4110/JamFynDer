...
const PlayerPage = () => {
  ...
  const REDIRECT_URI = "http://localhost:3000/PlayerPage";
          ...
          function () {
            ...
            setTimeout(() => {
              ...
              spotifyApi.getMyCurrentPlayingTrack().then(
                ...
                function (err) {
                  console.log(
                    "Something went wrong! - getMyCurrentPlayingTrack()",
                    err
                  );
                }
              );
      ...
      if (playlistUris_dic[i].track_uri === currentTrackURI) {
        ...
        for (let j = 0; j < genre_points.length; j++) {
          if (genre_points[j].genre === genre_match) {
            ...
            console.log(
              "Liked, " +
                genre_points[j].genre +
                " points are now at: " +
                genre_points[j].points
            );
            break;
          }
        }
          ...
          if (genre_points[j].genre === genre_match) {
            ...
            if (genre_points[j].points === 0) {
              console.log(
                "Disliked, " +
                  genre_points[j].genre +
                  " points is already at 0, can't subtract more points"
              );
            } else {
  ...
  useEffect(() => {
    ...
    spotifyApi
      ...
      .then(() => {
        spotifyApi
          .getPlaylist(hiphop_uri) // hip hop
          .then((res) => {
            res.body.tracks.items.slice(0, songLimit).map((trackUri) => {
              playlistUris_dic.push({
                track_uri: trackUri.track.uri,
          ...
          .then(() => {
            spotifyApi
              ...
              .then(() => {
                spotifyApi
                  .getPlaylist(bollywood_uri) // bollywood
                  .then((res) => {
                  ...
                  .then(() => {
                    ...
                    spotifyApi
                      ...
                      .then(
                        function (data) {
                          console.log(
                            "Playlist created with uri: " + data.body.uri
                          );
                        ...
                        function (err) {
                          console.log(
                            "Something went wrong - createPlaylist()!",
                            playlistCreatedState,
                            err
                          );
                        }
                      );
  ...
  const likeSong = () => {
    ...
    spotifyApi.getMyCurrentPlayingTrack().then(
      function (data) {
        ...
        spotifyApi
          .addTracksToPlaylist(JamFynDerPlaylistUri, [currentTrackURI])
          .then(
            () => {
  ...
  return (
    <div className="App">
      ...
      <button className="like" onClick={dislikeSong}>
      ...
      <div>
        {!token ? (
          ...
          <button classname="like" onClick={logout}>
            <span>Logout</span>
          </button>
        )}
        ...
        {token ? (
          <div>
            <h1>Select genres</h1>
            <button className="button" onClick={() => setCreatedSongList(true)}>
              <img
                class="goku"
                id="thing"
